import Word from '../models/words';

export const WORDS = [
    new Word(
        't1',
        'talk',
        'Hello',
        require('../assets/images/speechboard/talk/hello.png')
    ),
    new Word(
        't2',
        'talk',
        'Goodbye',
        require('../assets/images/speechboard/talk/goodbye.png')
    ),
    new Word(
        't3',
        'talk',
        'Thank You',
        require('../assets/images/speechboard/talk/thank_you.png')
    ),
    // new Word(
    //     't4',
    //     'talk',
    //     'No Thank You',
    //     require('../assets/images/speechboard/talk/cow.png')
    // ),
    new Word(
        't5',
        'talk',
        'No',
        require('../assets/images/speechboard/talk/no.png')
    ),
    new Word(
        't6',
        'talk',
        'Yes',
        require('../assets/images/speechboard/talk/yes.png')
    ),
    new Word(
        't7',
        'talk',
        'Okay',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't8',
        'talk',
        'How Are You?',
        null
    ),
    new Word(
        't9',
        'talk',
        'Who',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't10',
        'talk',
        'What',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't11',
        'talk',
        'Where',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't12',
        'talk',
        'When',
        require('../assets/images/speechboard/talk/when.png')
    ),
    new Word(
        't13',
        'talk',
        'Why',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't14',
        'talk',
        'How',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't15',
        'talk',
        'Bad',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        't16',
        'talk',
        'Good',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f1',
        'i feel',
        'I feel...',
        null
    ),
    new Word(
        'f2',
        'i feel',
        'I am...',
        null
    ),
    new Word(
        'f3',
        'i feel',
        'Happy',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f4',
        'i feel',
        'Sad',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f5',
        'i feel',
        'Angry',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f6',
        'i feel',
        'Excited',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f7',
        'i feel',
        'Tired',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f8',
        'i feel',
        'Scared',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f9',
        'i feel',
        'Hungry',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f10',
        'i feel',
        'Thirsty',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f11',
        'i feel',
        'Bored',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'am1',
        'about me',
        'My name is...',
        null
    ),
    new Word(
        'am2',
        'about me',
        'I am...',
        null
    ),
    new Word(
        'am3',
        'about me',
        'I am from...',
        null
    ),
    new Word(
        'a0',
        'activities',
        'I want to...',
        null
    ),
    new Word(
        'a1',
        'activities',
        'I like to...',
        null
    ),
    new Word(
        'a2',
        'activities',
        'Can we...',
        null
    ),
    new Word(
        'a3',
        'activities',
        'All done',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a4',
        'activities',
        'Play',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a5',
        'activities',
        'With toys',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a6',
        'activities',
        'With puzzles',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a7',
        'activities',
        'Swim',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a8',
        'activities',
        'Color',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a9',
        'activities',
        'Paint',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a10',
        'activities',
        'Draw',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a11',
        'activities',
        'Run',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a12',
        'activities',
        'Jump',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'a13',
        'activities',
        'Tickle',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f1',
        'food & drink',
        'I want to eat...',
        null
    ),
    new Word(
        'f2',
        'food & drink',
        'I want to eat...',
        null
    ),
    new Word(
        'f3',
        'food & drink',
        'I need...',
        null
    ),
    new Word(
        'f4',
        'food & drink',
        'Food',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f5',
        'food & drink',
        'Fruit',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f6',
        'food & drink',
        'Vegetable',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f7',
        'food & drink',
        'Drink',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f8',
        'food & drink',
        'Water',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f9',
        'food & drink',
        'Juice',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f10',
        'food & drink',
        'Milk',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f11',
        'food & drink',
        'Apple',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f12',
        'food & drink',
        'Orange',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f13',
        'food & drink',
        'Banana',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f14',
        'food & drink',
        'Chips',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f15',
        'food & drink',
        'Pasta',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f16',
        'food & drink',
        'Pizza',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f17',
        'food & drink',
        'Burger',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f18',
        'food & drink',
        'Sandwich',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f19',
        'food & drink',
        'Soup',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f20',
        'food & drink',
        'Mac and Cheese',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f21',
        'food & drink',
        'Crackers',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f22',
        'food & drink',
        'Dinner',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f23',
        'food & drink',
        'Breakfast',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f24',
        'food & drink',
        'Lunch',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'f25',
        'food & drink',
        'Dessert',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p1',
        'places',
        'Where are we going?',
        null
    ),
    new Word(
        'p2',
        'places',
        'Can we go...',
        null
    ),
    new Word(
        'p3',
        'places',
        'to',
        null
    ),
    new Word(
        'p4',
        'places',
        'Where is...',
        null
    ),
    new Word(
        'p5',
        'places',
        'home',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p6',
        'places',
        'the car',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p7',
        'places',
        'inside',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p8',
        'places',
        'outside',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p9',
        'places',
        'pool',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p10',
        'places',
        'playground',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p11',
        'places',
        'store',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p12',
        'places',
        'park',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p13',
        'places',
        'library',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'p14',
        'places',
        'school',
        require('../assets/images/speechboard/talk/cow.png')
    ),
    new Word(
        'c1',
        'colors',
        'My favorite color is...',
        null
    ),
    new Word(
        'c2',
        'colors',
        'What is your favorite color?',
        null
    ),
    new Word(
        'c3',
        'colors',
        'Red',
        require('../assets/images/speechboard/colors/red.png')
    ),
    new Word(
        'c4',
        'colors',
        'Orange',
        require('../assets/images/speechboard/colors/orange.png')
    ),
    new Word(
        'c5',
        'colors',
        'Yellow',
        require('../assets/images/speechboard/colors/yellow.png')
    ),
    new Word(
        'c6',
        'colors',
        'Blue',
        require('../assets/images/speechboard/colors/blue.png')
    ),
    new Word(
        'c7',
        'colors',
        'Green',
        require('../assets/images/speechboard/colors/green.png')
    ),
    new Word(
        'c8',
        'colors',
        'Purple',
        require('../assets/images/speechboard/colors/purple.png')
    ),
    new Word(
        'c9',
        'colors',
        'White',
        require('../assets/images/speechboard/colors/white.png')
    ),
    new Word(
        'c10',
        'colors',
        'Black',
        require('../assets/images/speechboard/colors/black.png')
    ),
    new Word(
        'c11',
        'colors',
        'Grey',
        require('../assets/images/speechboard/colors/grey.png')
    ),
    new Word(
        'c12',
        'colors',
        'Brown',
        require('../assets/images/speechboard/colors/brown.png')
    ),
];